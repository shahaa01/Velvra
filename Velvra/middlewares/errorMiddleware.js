...
const handleError = (err, req, res, next) => {
  ...
  if (err.code === 11000) {
    // Custom message for reviews unique index
    if (
      (err.message && err.message.includes('productId_1_userId_1')) ||
      (err.keyPattern && err.keyPattern.productId && err.keyPattern.userId)
    ) {
      error = new AppError('Sorry, you cannot post more than one review on a product.', 400);
    } else {
  ...
  if (isAPIRequest) {
    ...
    if (error.statusCode === 401) {
      ...
      if (referrer && !referrer.endsWith('/back')) {
