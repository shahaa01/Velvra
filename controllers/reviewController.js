...
exports.createOrUpdateReview = asyncWrap(async (req, res) => {
  ...
  const { id } = req.params;
  if (!id || !req.user._id) {
    throw new AppError('Product and user are required to submit a review.', 400);
  }
  ...
  // Log productId and userId before creating the review
  console.log('Creating review with:', { productId: id, userId: req.user._id, rating, comment, reviewerName, images });
